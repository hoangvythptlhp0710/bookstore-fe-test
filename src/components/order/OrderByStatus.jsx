        ...
        return (
            <div>
                ...
                {this.state.orders.length !== 0 &&
                    <div className="order_item"><hr></hr>{
                        this.state.orders.map(order => (<div key={order.id}><h2>Orders: {order.orderStatus}</h2><hr></hr>
                            ...
                            {(role === "ROLE_ADMIN" && order.orderStatus==="customer_confirmed") && <button onClick={()=>this.handleUpdateStatus(order.id, order.orderStatus, "admin_preparing")}>Prepare</button>}
                            {(role === "ROLE_ADMIN" && order.orderStatus==="admin_preparing") && <button onClick={()=>this.handleUpdateStatus(order.id, order.orderStatus, "shipping")}>Ship</button>}
                            {(role === "ROLE_CUSTOMER" && order.orderStatus ==="admin_preparing") && <button onClick={()=>this.handleUpdateStatus(order.id, order.orderStatus, "customer_request_cancel")}>Request cancel</button>}
                            {order.orderStatus ==="customer_confirmed" && <button onClick={()=>this.handleUpdateStatus(order.id, order.orderStatus, "customer_canceled")}>Cancel</button>}
                            {(role === "ROLE_CUSTOMER" && order.orderStatus ==="shipping") && <button onClick={()=>this.handleUpdateStatus(order.id, order.orderStatus, "success")}>Received</button>}
                            {(order.orderStatus === "success" || order.orderStatus === "customer_canceled") && <button>Buy again</button>}
                            <hr></hr>
                        </div>))
                    }
                    </div>
